...
client.once("ready", () => {
  ...
  if (targetChannelId) {
    console.log(
      `Listening for mentions in specific channel: ${targetChannelId}`
    );
  } else {
...
client.on("messageCreate", async (message) => {
  ...
  if (!question) {
    console.log(
      `Bot mentioned without a question by ${message.author.tag}. Ignoring.`
    );
  ...
  console.log(
    `Received question from channel ${channelId} by ${message.author.tag}: "${question}"`
  );
  ...
  const statusTimeout = setTimeout(() => {
    message.reply(
      "I'm still working on your request. This might take a bit longer than usual."
    );
  ...
  try {
    ...
    const responseFromN8n = await axios.post(
      n8nWebhookUrl,
      {
        question: question,
        channelId: channelId,
    ...
    console.log(
      "Complete response from n8n:",
      JSON.stringify(responseFromN8n.data)
    );
    ...
    if (responseFromN8n.data && responseFromN8n.data.answer) {
      ...
      Array.isArray(responseFromN8n.data) &&
      responseFromN8n.data[0]?.answer
    ) {
      // Array format: [{ "answer": "..." }]
      ...
      console.log(
        "No known response structure found, trying fallback parsing..."
      );
    ...
    if (answer) {
      ...
      if (answer.length <= 2000) {
        ...
        for (let i = 1; i < chunks.length; i++) {
          ...
          console.log(
            `Sent chunk ${i + 1} of ${chunks.length} (${
              chunks[i].length
            } chars)`
          );
      ...
      console.log(
        "Received empty or invalid answer structure from n8n:",
    ...
    message.reply(
      "Oops, something went wrong when communicating with n8n. Please try again later."
    );
